...
const Dashboard = () => {
  ...
  const handleExtractAttendanceByLevel = async () => {
    ...
    try {
      ...
      const endDate = format(
        new Date(Number(monthStart), Number(monthPart), 0),
        "yyyy-MM-dd",
      );
      ...
      if (attendanceError) {
        throw new Error(
          attendanceError.message || "Failed to load attendance records",
        );
      }
      ...
      const attendanceSet = new Set(
        ((attendanceData || []) as AttendanceForExtract[]).map(
          (record) =>
            `${record.member_id}|${record.attendance_date}|${record.service_type}`,
        ),
      );
  ...
  const loadStats = async () => {
    ...
    const totalFirstTimers =
      members?.filter((m) => m.is_first_timer).length || 0;
    ...
    const attendanceRate =
      totalMembers > 0
        ? Math.round((totalAttendanceThisMonth / totalMembers) * 100)
        : 0;
              ...
              <CarouselContent className="-ml-2">
                {widgets
                  ...
                  .map((widgetId) => (
                    <CarouselItem
                      key={widgetId}
                      className="pl-2 basis-[90%] xs:basis-[85%] sm:basis-1/2"
                    >
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Your dashboard is empty. Click "Customize" to reset to default
                layout.
              </p>
