  ...
  const onSubmit = async (data: MemberFormData) => {
    ...
    try {
      const { error } = await supabase.from("members").insert({
        ...
        promoted_to_member_at: data.is_first_timer
          ? null
      ...
      console.error("AddMemberDialog onSubmit error:", error);
      // Supabase returns a PostgrestError-like object; show useful details when available
      ...
      toast.error("Failed to add member: " + (message || "Unknown error"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <Label
                htmlFor="is_first_timer"
                className="flex items-center gap-2 cursor-pointer"
              >
                <input
                  type="checkbox"
                  id="is_first_timer"
                  {...register("is_first_timer")}
                  className="h-4 w-4"
                />
